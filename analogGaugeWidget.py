...
class AnalogGaugeWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.value_needle = QObject()  # 원래는 괄표가 없었음. 뭔 차이인지 모르겠음.
    ...
    def draw_filled_polygon(self, outline_pen_with=0):
        if not self.scale_polygon_colors == None:
            ...
            gauge_rect = QRect(QPoint(0, 0), QSize(
                self.widget_diameter / 2 - 1, self.widget_diameter - 1))
    ...
    def create_scale_marker_values_text(self):
        ...
        s_per_div_float = ((self.maxValue - self.minValue) / self.scalaCount)

        angle_distance = (float(self.scale_angle_size) /
                          float(self.scalaCount))
        for i in range(self.scalaCount + 1):
            # float value 가 필요한 경우를 위하여 ...
            if self.floatValue == False :
    ...
    def create_values_text(self):
        ...
        # float value 가 필요한 경우에 사용함.
        if self.floatValue == False:
            text = str(int(self.value))
        else :
            text = "{0:.1f}".format(self.value)   # VALUE 값의 실수 표현.
        w = fm.width(text) + 1
        h = fm.height()
